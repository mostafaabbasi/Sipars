...
(function ($) {
    ...
    function createDateTimePickerHtml($popoverDescriber, dateTimeInJsonFormat, writeDateString, initializing) {
        var triggerName = $popoverDescriber.attr('data-trigger'),
            ...
            todayDateTimeString = '<span style="color:blue;font-weight:bold;"> امروز </span>, ' + getPersianWeekDay(persianTodayDateTemp[2]) + ' ' + toPersianNumber(currentDayNumber) + ' ' + getPersianMonth(currentMonthNumber) + ' ' + toPersianNumber(currentYearNumber),
        ...
        for (i = 1; i <= numberOfDaysInCurrentMonth; i++) {
            ...
            if (fromDateToDateJson != undefined &&
                ((fromDateToDateJson.FromDateNumber != undefined && currentDateNumber < fromDateToDateJson.FromDateNumber))) {
               // alert(fromDateToDateJson.FromDateNumber);

                $td.attr('data-name', 'disabled-day');
            }

            if (fromDateToDateJson != undefined &&
              ((fromDateToDateJson.ToDateNumber != undefined && currentDateNumber > fromDateToDateJson.ToDateNumber))) {
                $td.attr('data-name', 'disabled-day');
            }
